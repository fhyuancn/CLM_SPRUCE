...
#ifdef MICROBE
  private :: init_column_microbe_type
  private :: init_gridcell_microbe_type
#endif
!EOP
!----------------------------------------------------
...
#ifdef MICROBE    
    allocate(decomp_cascade_con%is_microbe(0:ndecomp_pools))    
#endif
...
#ifdef MICROBE
    decomp_cascade_con%is_microbe(0:ndecomp_pools) = .false.
#endif
...
!#if (defined MICROBE) xiaofeng blocked this code
    ! CH4 code
 !   allocate(pps%grnd_ch4_cond(beg:end))
 !   allocate(pps%canopy_cond(beg:end))
!#endif
...
!------------------------------------------------------------------------
...
#if (defined MICROBE)
    allocate(pcf%m_frootc_to_doc(beg:end))
#endif
    ...
    allocate(pcf%excess_cflux_to_cpool(beg:end))
    allocate(pcf%cpool_to_turnover(beg:end))
    ...
    pcf%excess_cflux_to_cpool(beg:end) = nan
    pcf%cpool_to_turnover(beg:end) = nan
...
!------------------------------------------------------------------------
...
#if(defined MICROBE)
    allocate(pnf%m_frootn_to_don(beg:end))
#endif
    ...
    allocate(pnf%hrv_npool_to_litter(beg:end))       					    
    ...
    allocate(pnf%leafn_to_npool(beg:end))
    allocate(pnf%frootn_to_retransn(beg:end))
    allocate(pnf%frootn_to_npool(beg:end))
    ...
    pnf%hrv_npool_to_litter(beg:end) = nan        
   ...
   pnf%m_leafn_to_fire(beg:end) = nan
    ...
    pnf%leafn_to_npool(beg:end) = nan
    pnf%frootn_to_retransn(beg:end) = nan
    pnf%frootn_to_npool(beg:end) = nan
...
#ifdef MICROBE
! New variable for finundated parameterization
    allocate(cps%soilpH_unsat(beg:end, nlevgrnd))
    allocate(cps%soilpH_sat(beg:end, nlevgrnd))
#endif
...
#ifdef MICROBE
    allocate(ccs%micbiohr_col(beg:end))
    allocate(ccs%micbioc_col(beg:end))
    allocate(ccs%doc_col(beg:end))
    allocate(ccs%dochr_col(beg:end))
    allocate(ccs%dochr_vr(beg:end, 1:nlevdecomp_full))
    allocate(ccs%ace_col(beg:end))
    allocate(ccs%ace_prod_col(beg:end))
    allocate(ccs%acebios_col(beg:end))
    allocate(ccs%co2bio_col(beg:end))
    allocate(ccs%aerch4bio_col(beg:end))
    allocate(ccs%anaerch4bio_col(beg:end))  
    allocate(ccs%ccon_ch4s_col(beg:end))
    allocate(ccs%ccon_co2s_col(beg:end))    
#endif
...
#ifdef MICROBE
    allocate(cns%micbion_col(beg:end))
    allocate(cns%don_col(beg:end))
#endif
...
!------------------------------------------------------------------------
    ...
    !HUM_HOL
    allocate(cwf%qflx_surf_input(beg:end))
    allocate(cwf%qflx_lat_aqu(beg:end))
    allocate(cwf%qflx_lat_aqu_layer(1:2,beg:end))
...
!   allocate(ccf%cpool_turnover_to_litr_met_c(beg:end, 1:nlevdecomp_full))
#if(defined MICROBE)
    allocate(ccf%litter_c_to_doc(beg:end, 1:nlevdecomp_full))
    allocate(ccf%mic_fco2_col(beg:end))
    allocate(ccf%mic_fch4_col(beg:end))
#endif
#ifdef NITRIF_DENITRIF
...
!    ccf%cpool_turnover_to_litr_met_c(beg:end, 1:nlevdecomp_full)                 = nan
...
!BOP
!
! !IROUTINE: init_column_microbe_type
!
! !INTERFACE:
#ifdef MICROBE
  subroutine init_column_microbe_type(beg, end, cmic)
!
! !DESCRIPTION:
! Initialize column methane flux variables
!
...
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: beg, end
    type (column_microbe_type), intent(inout):: cmic
!
! !REVISION HISTORY:
! Created by William J. Riley
!
!EOP
!------------------------------------------------------------------------
    allocate(cmic%cmicbiocs(beg:end,1:nlevgrnd))
    allocate(cmic%cmicbions(beg:end,1:nlevgrnd))
    
    allocate(cmic%cdocs_pre(beg:end,1:nlevgrnd))
    allocate(cmic%cdocs(beg:end,1:nlevgrnd))
    allocate(cmic%cdocs_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%cdocs_sat(beg:end,1:nlevgrnd))  
    allocate(cmic%cdons(beg:end,1:nlevgrnd))
    allocate(cmic%cdons_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%cdons_sat(beg:end,1:nlevgrnd))  
    allocate(cmic%cdons_min(beg:end,1:nlevgrnd))  
    allocate(cmic%caces(beg:end,1:nlevgrnd))
    allocate(cmic%caces_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%caces_sat(beg:end,1:nlevgrnd))
    allocate(cmic%cacebios(beg:end,1:nlevgrnd))
    allocate(cmic%cacebios_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%cacebios_sat(beg:end,1:nlevgrnd))
    allocate(cmic%cco2bios(beg:end,1:nlevgrnd))
    allocate(cmic%cco2bios_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%cco2bios_sat(beg:end,1:nlevgrnd))
    allocate(cmic%caerch4bios(beg:end,1:nlevgrnd))
    allocate(cmic%caerch4bios_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%caerch4bios_sat(beg:end,1:nlevgrnd))    
    allocate(cmic%canaerch4bios(beg:end,1:nlevgrnd))
    allocate(cmic%canaerch4bios_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%canaerch4bios_sat(beg:end,1:nlevgrnd))
    
    allocate(cmic%dochr_vr(beg:end,1:nlevgrnd))
    allocate(cmic%micbio_hr_vr(beg:end,1:nlevdecomp_full,1:ndecomp_cascade_transitions))
    allocate(cmic%micbio_hr(beg:end,1:ndecomp_cascade_transitions))
    allocate(cmic%root2doc(beg:end,1:nlevgrnd))
    allocate(cmic%froot_r(beg:end,1:nlevgrnd))
    allocate(cmic%cn_microbe(beg:end,1:nlevgrnd))
    
    allocate(cmic%ccon_ch4s(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_o2s(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_co2s(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_h2s(beg:end,1:nlevgrnd))
    
    allocate(cmic%ccon_ch4s_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_o2s_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_co2s_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_h2s_unsat(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_ch4s_sat(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_o2s_sat(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_co2s_sat(beg:end,1:nlevgrnd))
    allocate(cmic%ccon_h2s_sat(beg:end,1:nlevgrnd))
...
!    allocate(cmic%zwt_ch4_unsat(beg:end))
    allocate(cmic%fsat_pre(beg:end))
    allocate(cmic%waterhead_unsat(beg:end))

!    allocate(cmic%totcolch4(beg:end))
!    allocate(cmic%fphr(beg:end,1:nlevgrnd))
    allocate(cmic%annsum_counter(beg:end))
    allocate(cmic%tempavg_somhr(beg:end))
    allocate(cmic%annavg_somhr(beg:end))
    allocate(cmic%tempavg_finrw(beg:end))
    allocate(cmic%annavg_finrw(beg:end))
    
    allocate(cmic%bgnpp_timestep(beg:end))
    allocate(cmic%bgnpp_avg(beg:end))
!    allocate(cmic%sif(beg:end))
!    allocate(cmic%o2stress_unsat(beg:end,1:nlevgrnd))
!    allocate(cmic%o2stress_sat(beg:end,1:nlevgrnd))
!    allocate(cmic%ch4stress_unsat(beg:end,1:nlevgrnd))
!    allocate(cmic%ch4stress_sat(beg:end,1:nlevgrnd))
!    allocate(cmic%qflx_surf_lag(beg:end))
!    allocate(cmic%finundated_lag(beg:end))
!    allocate(cmic%layer_sat_lag(beg:end,1:nlevgrnd))

	allocate(cmic%ch4_prod_ace_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_prod_co2_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_oxid_o2_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_oxid_aom_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_aere_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_dif_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_ebul_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_prod_ace_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_decomp_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_cons_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_ebul_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_aere_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_dif_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_cons_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_aere_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_aere_oxid_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_decomp_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_cons_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_dif_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_prod_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_cons_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_aere_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_diff_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_ebul_depth_unsat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_surf_aere_unsat(beg:end))
	...
	allocate(cmic%co2_surf_netflux_unsat(beg:end))
	allocate(cmic%o2_surf_aere_unsat(beg:end))
	allocate(cmic%o2_surf_dif_unsat(beg:end))
	allocate(cmic%o2_surf_netflux_unsat(beg:end))
	allocate(cmic%h2_surf_aere_unsat(beg:end))
	allocate(cmic%h2_surf_ebul_unsat(beg:end))
	allocate(cmic%h2_surf_dif_unsat(beg:end))
	allocate(cmic%h2_surf_netflux_unsat(beg:end))

	allocate(cmic%ch4_prod_ace_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_prod_co2_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_oxid_o2_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_oxid_aom_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_aere_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_dif_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_ebul_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_prod_ace_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_decomp_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_cons_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_ebul_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_aere_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%co2_dif_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_cons_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_aere_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_aere_oxid_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_decomp_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_cons_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%o2_dif_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_prod_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_cons_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_aere_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_diff_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%h2_ebul_depth_sat(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_surf_aere_sat(beg:end))
	allocate(cmic%ch4_surf_ebul_sat(beg:end))
	allocate(cmic%ch4_surf_dif_sat(beg:end))
	allocate(cmic%ch4_surf_netflux_sat(beg:end))
	allocate(cmic%co2_surf_aere_sat(beg:end))
	allocate(cmic%co2_surf_ebul_sat(beg:end))
	allocate(cmic%co2_surf_dif_sat(beg:end))
	allocate(cmic%co2_surf_netflux_sat(beg:end))
	allocate(cmic%o2_surf_aere_sat(beg:end))
	allocate(cmic%o2_surf_dif_sat(beg:end))
	allocate(cmic%o2_surf_netflux_sat(beg:end))
	allocate(cmic%h2_surf_aere_sat(beg:end))
	allocate(cmic%h2_surf_ebul_sat(beg:end))
	allocate(cmic%h2_surf_dif_sat(beg:end))
	allocate(cmic%h2_surf_netflux_sat(beg:end))

	allocate(cmic%ch4_prod_ace_depth(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_prod_co2_depth(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_oxid_o2_depth(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_oxid_aom_depth(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_aere_depth(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_dif_depth(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_ebul_depth(beg:end,1:nlevgrnd))
	allocate(cmic%co2_prod_ace_depth(beg:end,1:nlevgrnd))
	allocate(cmic%co2_decomp_depth(beg:end,1:nlevgrnd))
	allocate(cmic%co2_cons_depth(beg:end,1:nlevgrnd))
	allocate(cmic%co2_ebul_depth(beg:end,1:nlevgrnd))
	allocate(cmic%co2_aere_depth(beg:end,1:nlevgrnd))
	allocate(cmic%co2_dif_depth(beg:end,1:nlevgrnd))
	allocate(cmic%o2_cons_depth(beg:end,1:nlevgrnd))
	allocate(cmic%o2_aere_depth(beg:end,1:nlevgrnd))
	allocate(cmic%o2_aere_oxid_depth(beg:end,1:nlevgrnd))
	allocate(cmic%o2_decomp_depth(beg:end,1:nlevgrnd))
	allocate(cmic%o2_cons_depth(beg:end,1:nlevgrnd))
	allocate(cmic%o2_dif_depth(beg:end,1:nlevgrnd))
	allocate(cmic%h2_prod_depth(beg:end,1:nlevgrnd))
	allocate(cmic%h2_cons_depth(beg:end,1:nlevgrnd))
	allocate(cmic%h2_aere_depth(beg:end,1:nlevgrnd))
	allocate(cmic%h2_diff_depth(beg:end,1:nlevgrnd))
	allocate(cmic%h2_ebul_depth(beg:end,1:nlevgrnd))
	allocate(cmic%ch4_surf_aere(beg:end))
	allocate(cmic%ch4_surf_ebul(beg:end))
	allocate(cmic%ch4_surf_dif(beg:end))
	allocate(cmic%ch4_surf_netflux(beg:end))
	allocate(cmic%co2_surf_aere(beg:end))
	allocate(cmic%co2_surf_ebul(beg:end))
	allocate(cmic%co2_surf_dif(beg:end))
	allocate(cmic%co2_surf_netflux(beg:end))
	allocate(cmic%o2_surf_aere(beg:end))
	allocate(cmic%o2_surf_dif(beg:end))
	allocate(cmic%o2_surf_netflux(beg:end))
	allocate(cmic%h2_surf_aere(beg:end))
	allocate(cmic%h2_surf_ebul(beg:end))
    ...
    cmic%canaerch4bios(beg:end,1:nlevgrnd) = 1e-15
    cmic%caces(beg:end,1:nlevgrnd) = 0_r8
    cmic%cacebios_unsat(beg:end,1:nlevgrnd) = 1e-15
    cmic%cco2bios_unsat(beg:end,1:nlevgrnd) = 1e-15
    cmic%caerch4bios_unsat(beg:end,1:nlevgrnd) = 1e-15
    cmic%canaerch4bios_unsat(beg:end,1:nlevgrnd) = 1e-15
    cmic%caces_unsat(beg:end,1:nlevgrnd) = 1e-15
    cmic%cacebios_sat(beg:end,1:nlevgrnd) = 1e-15
    cmic%cco2bios_sat(beg:end,1:nlevgrnd) = 1e-15
    cmic%caerch4bios_sat(beg:end,1:nlevgrnd) = 1e-15
    cmic%canaerch4bios_sat(beg:end,1:nlevgrnd) = 1e-15
    cmic%caces_sat(beg:end,1:nlevgrnd) = 0_r8

    cmic%dochr_vr(beg:end,1:nlevgrnd) = 0_r8
    cmic%micbio_hr_vr(beg:end,1:nlevdecomp_full,1:ndecomp_cascade_transitions) = 0_r8
    cmic%micbio_hr(beg:end,1:ndecomp_cascade_transitions) = 0_r8
    cmic%root2doc(beg:end,1:nlevgrnd) = 0._r8
    cmic%froot_r(beg:end,1:nlevgrnd) = 0._r8
    cmic%cn_microbe(beg:end,1:nlevgrnd) = 10._r8
            
    cmic%ccon_ch4s(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_o2s(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_co2s(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_h2s(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_ch4s_unsat(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_o2s_unsat(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_co2s_unsat(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_h2s_unsat(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_ch4s_sat(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_o2s_sat(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_co2s_sat(beg:end,1:nlevgrnd) = 0_r8
    cmic%ccon_h2s_sat(beg:end,1:nlevgrnd) = 0_r8
...
!    cmic%zwt_ch4_unsat(beg:end) = nan
    ...
    cmic%waterhead_unsat(beg:end) = spval ! To detect first time-step
...
!    cmic%fphr(beg:end,1:nlevgrnd) = nan
    ...
    cmic%bgnpp_avg(beg:end) = spval ! To detect first year
!    cmic%sif(beg:end) = nan
!    cmic%o2stress_unsat(beg:end,1:nlevgrnd) = spval ! To detect file input
!    cmic%o2stress_sat(beg:end,1:nlevgrnd) = spval ! To detect file input
!    cmic%ch4stress_unsat(beg:end,1:nlevgrnd) = nan
!    cmic%ch4stress_sat(beg:end,1:nlevgrnd) = nan
!    cmic%qflx_surf_lag(beg:end) = spval ! To detect file input
!    cmic%finundated_lag(beg:end) = spval ! To detect file input
!    cmic%layer_sat_lag(beg:end,1:nlevgrnd) = spval ! To detect file input

	cmic%ch4_prod_ace_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_prod_co2_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_oxid_o2_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_oxid_aom_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_aere_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_dif_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_ebul_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%co2_prod_ace_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%co2_decomp_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%co2_cons_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%co2_ebul_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%co2_aere_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%co2_dif_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%o2_cons_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%o2_aere_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%o2_aere_oxid_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%o2_decomp_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%o2_cons_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%o2_dif_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%h2_prod_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%h2_cons_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%h2_aere_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%h2_diff_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%h2_ebul_depth_unsat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_surf_aere_unsat(beg:end) = spval
	...
	cmic%co2_surf_netflux_unsat(beg:end) = spval
	cmic%o2_surf_aere_unsat(beg:end) = spval
	cmic%o2_surf_dif_unsat(beg:end) = spval
	cmic%o2_surf_netflux_unsat(beg:end) = spval
	cmic%h2_surf_aere_unsat(beg:end) = spval
	cmic%h2_surf_ebul_unsat(beg:end) = spval
	cmic%h2_surf_dif_unsat(beg:end) = spval
	cmic%h2_surf_netflux_unsat(beg:end) = spval

	cmic%ch4_prod_ace_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_prod_co2_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_oxid_o2_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_oxid_aom_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_aere_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_dif_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_ebul_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%co2_prod_ace_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%co2_decomp_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%co2_cons_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%co2_ebul_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%co2_aere_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%co2_dif_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%o2_cons_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%o2_aere_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%o2_aere_oxid_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%o2_decomp_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%o2_cons_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%o2_dif_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%h2_prod_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%h2_cons_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%h2_aere_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%h2_diff_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%h2_ebul_depth_sat(beg:end,1:nlevgrnd) = spval
	cmic%ch4_surf_aere_sat(beg:end) = spval
	cmic%ch4_surf_ebul_sat(beg:end) = spval
	cmic%ch4_surf_dif_sat(beg:end) = spval
	cmic%ch4_surf_netflux_sat(beg:end) = spval
	cmic%co2_surf_aere_sat(beg:end) = spval
	cmic%co2_surf_ebul_sat(beg:end) = spval
	cmic%co2_surf_dif_sat(beg:end) = spval
	cmic%co2_surf_netflux_sat(beg:end) = spval
	cmic%o2_surf_aere_sat(beg:end) = spval
	cmic%o2_surf_dif_sat(beg:end) = spval
	cmic%o2_surf_netflux_sat(beg:end) = spval
	cmic%h2_surf_aere_sat(beg:end) = spval
	cmic%h2_surf_ebul_sat(beg:end) = spval
	cmic%h2_surf_dif_sat(beg:end) = spval
	cmic%h2_surf_netflux_sat(beg:end) = spval

	cmic%ch4_prod_ace_depth(beg:end,1:nlevgrnd) = spval
	cmic%ch4_prod_co2_depth(beg:end,1:nlevgrnd) = spval
	cmic%ch4_oxid_o2_depth(beg:end,1:nlevgrnd) = spval
	cmic%ch4_oxid_aom_depth(beg:end,1:nlevgrnd) = spval
	cmic%ch4_aere_depth(beg:end,1:nlevgrnd) = spval
	cmic%ch4_dif_depth(beg:end,1:nlevgrnd) = spval
	cmic%ch4_ebul_depth(beg:end,1:nlevgrnd) = spval
	cmic%co2_prod_ace_depth(beg:end,1:nlevgrnd) = spval
	cmic%co2_decomp_depth(beg:end,1:nlevgrnd) = spval
	cmic%co2_cons_depth(beg:end,1:nlevgrnd) = spval
	cmic%co2_ebul_depth(beg:end,1:nlevgrnd) = spval
	cmic%co2_aere_depth(beg:end,1:nlevgrnd) = spval
	cmic%co2_dif_depth(beg:end,1:nlevgrnd) = spval
	cmic%o2_cons_depth(beg:end,1:nlevgrnd) = spval
	cmic%o2_aere_depth(beg:end,1:nlevgrnd) = spval
	cmic%o2_aere_oxid_depth(beg:end,1:nlevgrnd) = spval
	cmic%o2_decomp_depth(beg:end,1:nlevgrnd) = spval
	cmic%o2_cons_depth(beg:end,1:nlevgrnd) = spval
	cmic%o2_dif_depth(beg:end,1:nlevgrnd) = spval
	cmic%h2_prod_depth(beg:end,1:nlevgrnd) = spval
	cmic%h2_cons_depth(beg:end,1:nlevgrnd) = spval
	cmic%h2_aere_depth(beg:end,1:nlevgrnd) = spval
	cmic%h2_diff_depth(beg:end,1:nlevgrnd) = spval
	cmic%h2_ebul_depth(beg:end,1:nlevgrnd) = spval
	cmic%ch4_surf_aere(beg:end) = spval
	cmic%ch4_surf_ebul(beg:end) = spval
	cmic%ch4_surf_dif(beg:end) = spval
	cmic%ch4_surf_netflux(beg:end) = spval
	cmic%co2_surf_aere(beg:end) = spval
	cmic%co2_surf_ebul(beg:end) = spval
	cmic%co2_surf_dif(beg:end) = spval
	cmic%co2_surf_netflux(beg:end) = spval
	cmic%o2_surf_aere(beg:end) = spval
	cmic%o2_surf_dif(beg:end) = spval
	cmic%o2_surf_netflux(beg:end) = spval
	cmic%h2_surf_aere(beg:end) = spval
	cmic%h2_surf_ebul(beg:end) = spval
	cmic%h2_surf_dif(beg:end) = spval
	cmic%h2_surf_netflux(beg:end) = spval

  end subroutine init_column_microbe_type
#endif
...
#if(defined MICROBE)
    allocate(cnf%litter_n_to_don(beg:end, 1:nlevdecomp_full))
#endif
...
#ifdef MICROBE
  subroutine init_gridcell_microbe_type(beg, end, gmic)
!
! !DESCRIPTION:
! Initialize gridcell ch4 variables
!
...
! !ARGUMENTS:
    implicit none
    integer, intent(in) :: beg, end
    type (gridcell_microbe_type), intent(inout):: gmic
!
! !REVISION HISTORY:
! Created by Mariana Vertenstein
!
!EOP
!------------------------------------------------------------------------
    allocate(gmic%c_atm(beg:end,1:ngases))
!    allocate(gmic%gaerch4co2f(beg:end))
!    allocate(gmic%ganaerch4co2f(beg:end))
!    allocate(gmic%gch4prodf(beg:end))
!    allocate(gmic%nem(beg:end))

    gmic%c_atm(beg:end,1:ngases) = nan
!    gmic%gaerch4co2f(beg:end) = nan
!    gmic%ganaerch4co2f(beg:end) = nan
!    gmic%gch4prodf(beg:end) = nan
!    gmic%nem(beg:end) = nan

  end subroutine init_gridcell_microbe_type
#endif
